...
import iwls
from iwls import _compute_betas_gwr
...
class GWR(GLM):
    ...
    def __init__(self, coords, y, X, bw, lwcc=False, family=Gaussian(), offset=None,
        ...
        if lwcc:
            constant=False # Force no intercept if lwcc=True. 
    ...
    def _local_fit(self, i): # modified local fit to enforce slope-correlation identity
        ...
        w_self = wi[i][0] # retain location i's self weight


        if isinstance(self.family, Gaussian):
            if self.lwcc == True:
                betas, inv_xtx_xt, X_std, Xw_std = _compute_betas_gwr(self.y, self.X, self.lwcc, wi)
                predy = np.dot(X_std[i], betas)[0]
                # predy = (predy * self.ystd) + self.ymean
                resid = self.y[i] - predy
                influ = np.dot(Xw_std[i], inv_xtx_xt[:, i])

                Si = np.dot(Xw_std[i], inv_xtx_xt).reshape(-1) # influence must use weighted>>standardize

                tr_STS_i = np.sum(Si * Si * w_self * w_self) # correct weight use
                ...
                if self.fit_params['lite']:
                    ...
                    if not self.hat_matrix:
                        Si = None
                    return influ, resid, predy, betas.reshape(-1), w_self, Si, tr_STS_i, CCT
                ...
                # Correct influence vector (use weighted X)
                W_sqrt = np.sqrt(wi).flatten()
                Xw = self.X * W_sqrt[:, None]
                Si = np.dot(Xw[i], inv_xtx_xt).reshape(-1)

                # Correct tr(STS): NO w_self^2
