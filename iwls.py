...
from spglm.family import Binomial, Poisson
...
def _compute_betas_gwr(y, x, lwcc, wi):
    ...
    Modified: 11/26/2025 by Victor Irekponor vireks@terpmail.umd.edu
    
    """
    if lwcc:
        # Flatten weights to 1D for convenience
        w = wi.flatten()                     # shape (n,)
        w_sum = w.sum()
        ...
        # Weighted means
        mx = (w_norm[:, None] * X).sum(axis=0)   # (k,)
        my = (w_norm * Y.flatten()).sum()        # scalar
        ...
        # Weighted std devs
        sx = np.sqrt((w_norm[:, None] * Xc**2).sum(axis=0))   # (k,)
        sy = np.sqrt((w_norm * (Yc.flatten()**2)).sum())      # scalar

        # Standardize (locally, with weights)
        X_std = Xc / sx                       # (n, k)
        Y_std = Yc / sy                       # (n, 1)

        # Now do weighted least squares with same weights w
        # W^(1/2) * X_std, W^(1/2) * Y_std
        W_sqrt = np.sqrt(w)[:, None]         # (n, 1)
        Xw_std = X_std * W_sqrt              # (n, k)
        Yw_std = Y_std * W_sqrt              # (n, 1)

        XtW = Xw_std.T                       # (k, n)
        XtX = np.dot(XtW, Xw_std)            # (k, k)
        XtX_inv_Xt = linalg.solve(XtX, XtW)  # (k, n)
        betas = np.dot(XtX_inv_Xt, Yw_std)   # (k, 1)

        return betas, XtX_inv_Xt, X_std, Xw_std     # <- return standardized X (no weights)
    else:
        xT = (x * wi).T
        xtx = np.dot(xT, x)
